...
async function unwrap<T>(promise: Promise<{ data: T; status: number }>, step: string): Promise<T> {
...
export async function readCard(waitTime = 10): Promise<any> {
  return unwrap(payterClient.get("/card", { params: { waitTime } }), "Read card");
}

export async function authorizeSession(): Promise<{ sessionId: string; authorizedAmount: number }> {
  return unwrap(payterClient.post("/authorize"), "Authorize session");
}

export async function commitSession(sessionId: string, commitAmount: number): Promise<any> {
  // adds UI message via params
  return unwrap(
...
export async function stopTerminal(uiMessage = "Stopped", uiMessageTimeout = 30): Promise<void> {
  await unwrap(
